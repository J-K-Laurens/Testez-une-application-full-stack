describe('Sessions - Gestion des sessions', () => {
  ...
  const loginAsAdmin = () => {
    cy.visit('/login');
    cy.intercept('POST', '/api/auth/login', { statusCode: 200, body: adminUser }).as('loginAdmin');
  ...
  const loginAsUser = () => {
    cy.visit('/login');
    cy.intercept('POST', '/api/auth/login', { statusCode: 200, body: regularUser }).as('loginUser');
    cy.intercept('GET', '/api/session', mockSessions).as('getSessions');
